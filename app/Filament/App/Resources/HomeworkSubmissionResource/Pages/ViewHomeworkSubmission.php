                                ...
                                if ($file instanceof \Livewire\Features\SupportFileUploads\TemporaryUploadedFile) {
                                    ...
                                    if ($isImage) {
                                        try {
                                            ...
                                            $newPath = 'feedback-attachments/' . uniqid() . '_' . time() . '.' . $extension;
                                            ...
                                            \Log::error('Feedback attachment resize failed: ' . $e->getMessage());
                                        }
                                    }
                                    // Non-images or failed resize
                                    \Illuminate\Support\Facades\Storage::disk('s3')->putFileAs('feedback-attachments', $file, basename($file->getFilename()), 'public');
                                    $processedState[] = 'feedback-attachments/' . basename($file->getFilename());
                                    $hasChanges = true;
                                } else {
                                    $processedState[] = $file;
                                }
