            ...
            ->schema([
                ...
                Forms\Components\FileUpload::make('presentations')
                    ...
                    ->afterStateUpdated(function (\Filament\Forms\Get $get, \Filament\Forms\Set $set, $state) {
                            ...
                            if ($file instanceof \Livewire\Features\SupportFileUploads\TemporaryUploadedFile) {
                                ...
                                if ($isImage) {
                                    try {
                                        ...
                                        $newPath = 'presentations/' . uniqid() . '_' . time() . '.' . $extension;
                                        ...
                                        \Log::error('Presentation image resize failed: ' . $e->getMessage());
                                    }
                                }
                                // Non-images or failed resize
                                \Illuminate\Support\Facades\Storage::disk('s3')->putFileAs('presentations', $file, basename($file->getFilename()), 'public');
                                $processedState[] = 'presentations/' . basename($file->getFilename());
                                $hasChanges = true;
                            } else {
                                $processedState[] = $file;
                            }
                        ...
                        if ($hasChanges) {
                            $set('presentations', $processedState);
                        }
                    })
                    ->columnSpanFull(),
                Forms\Components\Select::make('participants')
                    ...
                    ->afterStateUpdated(function (Forms\Set $set, Forms\Get $get, ?array $state) {
                        ...
                        if ($count === 0) {
                            // No students selected - set pending type
                            $set('type', 'pending');
                            $set('base_price', null);
